...
try:
    def fetch_dynamic_query(self, query: str, max_results: int = 50, prefer_primary: bool = False):
        """Fetch live signals for a query from Anakin actions and public sources.
        - Aggregates results from multiple Anakin actions (if configured).
        - Expands public-source fetches (Google News, arXiv, GitHub) for depth.
        - Deduplicates by (url, title) and trims to `max_results`.
        """
        ...
        # Aggregate Anakin-backed actions (don't break on first; collect from all configured actions)
        if ANAKIN_API_KEY:
        ...
        try:
            ...
            for entry in feed.entries[:min(30, max_results)]:  # Top articles
                s = {
                    "type": "news",
                    "source": "Google News",
                    "title": entry.title,
        ...
        # Yahoo Finance (company signals) — keep but dedupe
        ...
        # arXiv (academic) — expand to a few results
        ...
        # GitHub (developer tooling) — expand search
    ...
    def fetch_dynamic_query(self, query: str):
        ...
        # Optionally prefer primary sources (research, official, financial) by re-ordering
        if prefer_primary:
